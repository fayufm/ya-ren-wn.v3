...
// 保存修复脚本
fs.writeFileSync('fix-raw-verify.sh', fixScript);
console.log('已创建验证服务修复脚本');

// 上传并执行修复脚本
try {
  console.log('上传并执行验证服务修复脚本...');
  execSync('scp -i "C:\\Users\\34260\\.ssh\\id_rsa" fix-raw-verify.sh root@8.155.16.247:/tmp/');
  execSync('ssh -i "C:\\Users\\34260\\.ssh\\id_rsa" root@8.155.16.247 "chmod +x /tmp/fix-raw-verify.sh && /tmp/fix-raw-verify.sh"');
  console.log('验证服务已修复');
} catch (err) {
  console.error('修复验证服务失败:', err.message);
} 