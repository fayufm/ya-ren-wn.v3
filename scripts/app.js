...
let isProcessingImage = false; // 跟踪图片处理状态
let isProcessingAdditionalFiles = false; // 跟踪额外文件处理状态
...
// 格言生成辅助函数 - 当格言库不够用时可以组合生成新的格言
function generateClassicalSaying() {
  // 用于生成新格言的词组库
  const firstParts = [
    "君子", "仁者", "智者", "圣人", "贤者", "天道", "大道", "人生", 
    "为学", "修身", "处世", "立身", "志士", "大丈夫"
  ];
  ...
  const lastParts = [
    "仁德", "诚信", "礼义", "智慧", "节制", "勇毅", "孝悌", 
    "谦逊", "宽容", "和合", "正道", "大同", "中庸", "天理"
  ];
  ...
  switch(structure) {
    case 0: // 简单结构：君子尚仁德
      return firstParts[Math.floor(Math.random() * firstParts.length)] + 
             ...
             lastParts[Math.floor(Math.random() * lastParts.length)];
    
    case 1: // 对偶结构：君子尚仁德，贤者求智慧
      return firstParts[Math.floor(Math.random() * firstParts.length)] + 
             middleParts[Math.floor(Math.random() * middleParts.length)] + 
             lastParts[Math.floor(Math.random() * lastParts.length)] + "，" +
    ...
    case 2: // 因果结构：君子尚仁德，故行于正道
      return firstParts[Math.floor(Math.random() * firstParts.length)] + 
             middleParts[Math.floor(Math.random() * middleParts.length)] + 
             lastParts[Math.floor(Math.random() * lastParts.length)] + "，" +
             conjunctions[Math.floor(Math.random() * conjunctions.length)] + 
             middleParts[Math.floor(Math.random() * middleParts.length)] + 
             "于" + lastParts[Math.floor(Math.random() * lastParts.length)];
    
    case 3: // 并列结构：尚仁德，守诚信，行于正道
      return middleParts[Math.floor(Math.random() * middleParts.length)] + 
             lastParts[Math.floor(Math.random() * lastParts.length)] + "，" +
             middleParts[Math.floor(Math.random() * middleParts.length)] + 
             lastParts[Math.floor(Math.random() * lastParts.length)] + "，" +
             "行于" + lastParts[Math.floor(Math.random() * lastParts.length)];
  }
}
...
const MESSAGE_DAILY_LIMIT = 20; // 每天最多发送20条留言
...
function showTab(tab) {
  ...
  if (tab === 'home') {
    ...
    // 确保清空可能存在的旧数据
    if (commissionsList) {
      commissionsList.innerHTML = '<div class="spinner"></div><div class="loading-text">加载中...</div>';
    }
    
    // 强制刷新委托列表数据
    setTimeout(() => {
      loadCommissions().catch(err => console.error('加载委托失败:', err));
    }, 50);
  } else if (tab === 'my') {
    ...
    // 确保清空可能存在的旧数据
    if (myCommissions) {
      myCommissions.innerHTML = '<div class="spinner"></div><div class="loading-text">加载中...</div>';
    }
    if (myMessages) {
      myMessages.innerHTML = '<div class="spinner"></div><div class="loading-text">加载中...</div>';
    }
    
    // 使用Promise.all并行加载但处理各自的错误
    setTimeout(() => {
      // 先加载委托，然后加载消息，避免同时发起太多请求
      loadMyCommissions()
        .catch(err => console.error('加载我的委托失败:', err))
        .finally(() => {
          // 委托加载完成后，无论成功失败，都加载消息
          setTimeout(() => {
            loadMyMessages().catch(err => console.error('加载消息记录失败:', err));
          }, 100);
        });
    }, 50);
  } else if (tab === 'create') {
...
async function loadCommissions() {
  try {
    ...
    // 确保图片加载完整
    await preloadCommissionImages(commissions);
    ...
    // 显示更友好的错误提示
    if (commissionsList) {
      commissionsList.innerHTML = '<div class="empty-message">加载委托失败，请点击<a href="#" onclick="loadCommissions(); return false;">这里</a>重试</div>';
    } else {
...
// 预加载委托图片，确保图片加载不会失败
async function preloadCommissionImages(commissions) {
  // 创建图片加载Promise数组
  const imagePromises = commissions
    .filter(comm => comm.image) // 只处理有图片的委托
    .map(comm => {
  ...
  if (imagePromises.length > 0) {
    // 使用Promise.all等待所有图片加载，但超时10秒后继续
    await Promise.race([
...
function filterCommissionsByLocation(commissions, location) {
  if (!commissions || !Array.isArray(commissions)) {
    console.error('筛选委托失败：无效的委托数据', commissions);
    return [];
  }
  ...
  // 确保在比较前转换为字符串，避免类型不匹配问题
  return commissions.filter(commission => {
    // 防止空委托或无city属性
    if (!commission || typeof commission !== 'object') return false;
    
    // 比较城市时转为字符串并忽略大小写
    return String(commission.city || '').toLowerCase() === String(location).toLowerCase();
  });
}
...
async function loadMyCommissions() {
  try {
    ...
    // 显示更友好的错误提示
    if (myCommissions) {
      myCommissions.innerHTML = '<div class="empty-message">加载个人委托失败，请点击<a href="#" onclick="loadMyCommissions(); return false;">这里</a>重试</div>';
    } else {
...
async function updateDailyLimits() {
  try {
    ...
    const remainingCommissions = COMMISSION_DAILY_LIMIT - commissionLimitStatus.dailyCount;
    
    // 获取留言限制信息
    const messageLimitStatus = await checkMessageLimit();
    const remainingMessages = MESSAGE_DAILY_LIMIT - messageLimitStatus.dailyCount;
    
    // 更新界面显示
    document.getElementById('commission-limit-value').textContent = 
      `${remainingCommissions}/${COMMISSION_DAILY_LIMIT}`;
    document.getElementById('message-limit-value').textContent = 
      `${remainingMessages}/${MESSAGE_DAILY_LIMIT}`;
    ...
    document.getElementById('limit-refresh-time').textContent = 
      `下次刷新时间：${hours}小时${minutes}分钟后`;
      
    // 高亮显示接近限制的数值
    if (remainingCommissions <= 1) {
      document.getElementById('commission-limit-value').style.color = 'var(--danger-color)';
    } else {
      document.getElementById('commission-limit-value').style.color = 'var(--primary-color)';
    }
    
    if (remainingMessages <= 5) {
      document.getElementById('message-limit-value').style.color = 'var(--danger-color)';
    } else {
      document.getElementById('message-limit-value').style.color = 'var(--primary-color)';
    }
  } catch (error) {
    console.error('更新每日限额信息失败:', error);
    document.getElementById('commission-limit-value').textContent = '--/--';
    document.getElementById('message-limit-value').textContent = '--/--';
  }
}
...
async function loadMyMessages() {
  // 防止重复加载
  if (myMessages.dataset.loading === 'true') {
    return;
  }
  
  myMessages.dataset.loading = 'true';
  myMessages.innerHTML = '<div class="spinner"></div><div class="loading-text">加载中...</div>';
  ...
  try {
    ...
    for (const commission of commissions) {
      ...
      commissionPromises.push(messagePromise);
    }
    
    // 等待所有消息加载完成
    const commissionElements = await Promise.all(commissionPromises);
    
    // 过滤掉null元素并添加到容器
    commissionElements.filter(el => el !== null).forEach(el => {
      messagesContainer.appendChild(el);
    });
    
    myMessages.innerHTML = ''; // 清除加载状态
    myMessages.appendChild(messagesContainer);
    
    // 如果没有有效消息，显示提示
    if (!hasValidMessages || messagesContainer.children.length === 0) {
    ...
    // 自定义错误信息，避免使用弹窗，防止文言文模式下的翻译问题
    const errorMessage = classicalChineseMode ? 
      '音讯载入不遂，可点此重试' : 
      '加载消息记录失败，请点击这里重试';
      
    myMessages.innerHTML = `<div class="empty-message">${errorMessage} <a href="#" onclick="loadMyMessages(); return false;">🔄</a></div>`;
  } finally {
    // 确保无论如何都会重置加载状态
    myMessages.dataset.loading = 'false';
  }
}
...
async function showCommissionDetail(id) {
  ...
  try {
    ...
    if (!commission || commission.error) {
      showCustomAlert('此委托不存在或已被删除', '委托已失效');
      
      // 如果是从"我的"页面打开的，则刷新消息列表，清除已删除委托的留言
    ...
    if (commission.expiryDate) {
      ...
      if (commission.status === 'expired') {
        ...
        dateInfo += `<br><span class="expiry-info">截至日期：${expiryDateStr}（剩余 ${daysLeft} 天）</span>`;
      }
    }
  ...
  return new Promise((resolve, reject) => {
    try {
        ...
        try {
          ...
          img.onload = function() {
            try {
              ...
              // 如果图片尺寸太小，不需要压缩
              if (width <= 800 && height <= 800 && file.size < 1024 * 1024) {
                // 对于小图片，直接返回原始文件
                console.log('图片尺寸较小，跳过压缩');
                resolve(file);
                return;
              }
              ...
              if (!compressedDataUrl || compressedDataUrl === 'data:,') {
                console.error('Canvas输出无效');
                throw new Error('Canvas输出无效');
              }
              ...
              if (compressedFile.size > file.size) {
                ...
                console.log(`图片压缩成功: ${file.size} -> ${compressedFile.size}`);
      ...
      reader.onerror = function(readerError) {
        console.error('文件读取错误:', readerError);
        // 如果读取失败，返回原始文件
        resolve(file);
      };
    } catch (error) {
      console.error('图片压缩过程发生错误:', error);
      // 返回原始文件，确保上传功能不会中断
      resolve(file);
    }
  });
...
async function handleImageUpload(event) {
  ...
  if (!supportedFormats.includes(file.type)) {
    showCustomAlert('不支持的图片格式，请使用JPG、PNG或GIF格式', '格式错误');
    return;
  }
  
  // 添加图片大小限制检查：8MB = 8 * 1024 * 1024 字节
  const MAX_FILE_SIZE = 8 * 1024 * 1024; // 8MB
  if (file.size > MAX_FILE_SIZE) {
    showCustomAlert('图片大小不能超过8MB', '文件过大');
    return;
  }
  ...
  try {
    ...
    // 显示压缩中提示
    const loadingToast = showToast('正在处理图片...');
  ...
  reader.onload = function(e) {
    ...
    updateImagePreview();
      ...
      if (processedFile.size < file.size * 0.9) {
        const savedSize = ((file.size - processedFile.size) / 1024).toFixed(1);
        showToast(`图片已优化，节省了${savedSize}KB空间`);
      } else {
        showToast('图片已处理完成');
      }
    ...
    reader.onerror = function(error) {
      ...
      showCustomAlert('图片处理后读取失败，请重试', '处理错误');
    ...
    showCustomAlert('图片处理失败，请重试。错误信息: ' + (error.message || '未知错误'), '处理错误');
...
function selectLocation(locationElement) {
  ...
  // 根据当前视图重新加载并筛选委托列表
  if (homeView.classList.contains('active')) {
    loadCommissions(); // 如果在主页，重新加载所有委托
  } else if (myView.classList.contains('active')) {
    loadMyCommissions(); // 如果在"我的"页面，重新加载我的委托
  } else {
    // 在其他视图，至少刷新主页委托列表以备用
    window.api.getCommissions().then(commissions => {
      const filteredCommissions = filterCommissionsByLocation(commissions, currentLocation);
      renderCommissionsList(filteredCommissions, commissionsList);
    }).catch(error => {
      console.error('加载委托失败:', error);
...
commissionForm.addEventListener('submit', async (e) => {
  ...
  if (isProcessingImage) {
    showCustomAlert('图片正在处理中，请稍后再提交', '请稍候');
    return;
  }
  
  // 检查额外文件是否正在处理中
  if (isProcessingAdditionalFiles) {
    showCustomAlert('文件正在处理中，请稍后再提交', '请稍候');
    return;
  }
  ...
  try {
    ...
    console.log('内容审核结果:', contentCheckResult);
    ...
    if (currentImageData && currentImageData.length > 1024 * 1024 * 5) {
      // 如果图片大于5MB，尝试进一步压缩
      console.log('图片数据过大，尝试进一步压缩');
      try {
        optimizedImageData = await compressImageData(currentImageData, 0.6);
      } catch (imgError) {
        console.error('图片进一步压缩失败:', imgError);
        // 继续使用原始图片，但可能会失败
      }
    }
    ...
    console.log('准备发布委托，数据大小:', {
      title: title.length,
      description: description.length,
      contacts: JSON.stringify(contacts).length,
      imageSize: optimizedImageData ? optimizedImageData.length : 0,
      filesCount: additionalFiles ? additionalFiles.length : 0
    });
    ...
    try {
      ...
      // 先创建基础委托
      newCommission = await window.api.createCommission(baseCommission);
      
      // 检查是否有错误
      if (newCommission.error) {
        throw new Error(newCommission.message || '创建委托失败');
      }
      
      console.log('委托创建成功，ID:', newCommission.id);
      
    } catch (innerError) {
      console.error('创建基础委托失败:', innerError);
    ...
    if (document.body.contains(publishingAlert)) {
      ...
      if (innerError.message) {
        showCustomAlert(`发布委托失败: ${innerError.message}`, '错误');
      } else {
    ...
    showCustomAlert(`发布委托失败: ${error.message || '请稍后再试'}`, '错误');
  }
});

// 进一步压缩图片数据的辅助函数
async function compressImageData(dataUrl, quality = 0.7) {
  return new Promise((resolve, reject) => {
    try {
      const img = new Image();
      img.onload = function() {
...
async function checkMessageLimit() {
  try {
    // 获取所有委托的消息
    const allCommissions = await window.api.getCommissions();
    const allMessages = [];
    ...
    for (const commission of allCommissions) {
      ...
      if (messages && Array.isArray(messages)) {
        ...
        allMessages.push(...todayMessages);
      }
    }
    
    // 统计今天发送的留言数量
    const dailyCount = allMessages.length;
    const dailyLimitReached = dailyCount >= MESSAGE_DAILY_LIMIT;
...
async function deleteCommission(id) {
  try {
    ...
    // 如果当前在"我的"页面，同时刷新消息列表，确保已删除委托的留言不再显示
    if (document.getElementById('my-view').classList.contains('active')) {
      // 延迟一小段时间以确保服务器端处理完成
      setTimeout(() => {
        loadMyMessages();
...
async function initApp() {
  ...
  if (expiryDateInput) {
    ...
    expiryDateInput.max = maxDateStr;
  }
}
...
async function sendChatMessage() {
  ...
  // 检查留言数量限制
  const messageLimitStatus = await checkMessageLimit();
  if (messageLimitStatus.dailyLimitReached) {
    showCustomAlert(`您今天已经发送了${MESSAGE_DAILY_LIMIT}条留言，请明天再来留言`, '留言次数已达上限');
    return;
  }
...
function activateClassicalChineseMode() {
  ...
  // 显示切换中提示
  showToast('正在切换为文言文模式...');
  
  // 为了确保UI响应，延迟执行复杂的切换流程
  setTimeout(() => {
    ...
    // 完全重建定位按钮区域，使用文言文模式风格
    rebuildLocationElementsClassical();
    
    // 延长等待时间，确保DOM完全更新后再加载委托列表
    setTimeout(() => {
      // 在加载委托前先清空当前列表，避免显示旧数据
      if (commissionsList) {
        commissionsList.innerHTML = '<div class="spinner"></div><div class="loading-text">加载中...</div>';
      }
      ...
      // 完整重新初始化所有数据
      // 1. 重新加载主页委托
      loadCommissions().catch(err => console.error('切换模式后加载委托失败:', err));
      
      // 2. 如果当前在"我的"页面，也重新加载我的委托和消息
      if (document.getElementById('my-view').classList.contains('active')) {
        setTimeout(() => {
          loadMyCommissions().catch(err => console.error('切换模式后加载我的委托失败:', err));
          setTimeout(() => {
            loadMyMessages().catch(err => console.error('切换模式后加载消息失败:', err));
          }, 300);
        }, 200);
      }
      
      // 3. 如果当前在详情页面，重新加载详情
      if (document.getElementById('detail-view').classList.contains('active') && currentCommissionId) {
        showCommissionDetail(currentCommissionId);
      }
    }, 500); // 给DOM更新和资源加载留出足够时间
  }, 100); // 低延迟，提高UI响应速度
}

// 以文言文风格重建定位区域元素
function rebuildLocationElementsClassical() {
  try {
    // 保存原始定位元素的容器
    ...
    // 保存原始位置下拉菜单内容
    const originalOptionsList = Array.from(document.querySelectorAll('.location-option'));
    const selectedLocation = originalOptionsList.find(option => option.classList.contains('selected'))?.dataset.value || '全国';
    ...
    locationButton.className = 'location-circle-btn';
    // 文言文模式留空，通过CSS ::before添加"方域"文字
    ...
    dropdownHeader.className = 'location-dropdown-header';
    
    const headerSpan = document.createElement('span');
    headerSpan.textContent = '择地';
    
    const closeButton = document.createElement('button');
    closeButton.id = 'location-close';
    closeButton.className = 'location-close-btn';
    ...
    optionsContainer.className = 'location-options';
    
    // 重新创建所有选项
    // 常用地区
    const commonCategory = document.createElement('div');
    commonCategory.className = 'location-category';
    commonCategory.textContent = '常用';
    optionsContainer.appendChild(commonCategory);
    
    const allChinaOption = createLocationOption('全国', selectedLocation === '全国');
    optionsContainer.appendChild(allChinaOption);
    
    // 直辖市
    const municipalCategory = document.createElement('div');
    municipalCategory.className = 'location-category';
    municipalCategory.textContent = '直辖市';
    optionsContainer.appendChild(municipalCategory);
    
    ['北京', '上海', '天津', '重庆'].forEach(city => {
      const option = createLocationOption(city, selectedLocation === city);
      optionsContainer.appendChild(option);
    });
    
    // 将所有元素添加到DOM
    locationDropdown.appendChild(dropdownHeader);
    locationDropdown.appendChild(optionsContainer);
    
    locationFilter.appendChild(locationButton);
    locationFilter.appendChild(locationDropdown);
    ...
    document.querySelectorAll('.location-option').forEach(option => {
      option.addEventListener('click', () => {
        ...
        // 更新当前地区
        currentLocation = option.dataset.value;
        ...
        // 显示提示 - 文言文风格
        showToast(`已设${currentLocation}为所在地`);
      });
    });
    
    // 点击外部关闭下拉菜单
    document.addEventListener('click', (e) => {
      if (locationDropdown && !locationDropdown.contains(e.target) && e.target !== locationButton) {
        locationDropdown.classList.remove('active');
      }
    });
    
    console.log('文言文模式定位区域元素已重建');
  } catch (error) {
    console.error('重建文言文模式定位区域失败:', error);
  }
}
...
function deactivateClassicalChineseMode() {
  ...
  setTimeout(() => {
    ...
    }, 500); // 增加到500ms，给DOM更新和资源加载留出足够时间
  }, 100); // 降低主延迟，提高UI响应速度
}

// 彻底重建定位区域相关元素
function rebuildLocationElements() {
  try {
    // 保存原始定位元素的容器
    const locationFilter = document.querySelector('.location-filter');
    if (!locationFilter) return;
    ...
    locationButton.id = 'location-button';
    locationButton.className = 'location-circle-btn';
    locationButton.innerHTML = '<img src="dingwei.png" alt="定位" class="icon-img">';
    ...
    document.querySelectorAll('.location-option').forEach(option => {
      option.addEventListener('click', () => {
        ...
        // 显示提示
        showToast(`已切换到${currentLocation}地区`);
      });
    });
    ...
    console.log('定位区域元素已重建');
  } catch (error) {
    console.error('重建定位区域失败:', error);
  }
}

// 创建地区选项辅助函数
function createLocationOption(name, isSelected = false) {
  const option = document.createElement('div');
  option.className = 'location-option' + (isSelected ? ' selected' : '');
...
function cleanupEventListeners() {
  ...
  if (locationButton) {
    const newLocationButton = locationButton.cloneNode(true);
    locationButton.parentNode.replaceChild(newLocationButton, locationButton);
    
    // 立即重新绑定定位按钮点击事件
    document.getElementById('location-button').addEventListener('click', toggleLocationDropdown);
  }
  
  if (locationClose) {
    const newLocationClose = locationClose.cloneNode(true);
    locationClose.parentNode.replaceChild(newLocationClose, locationClose);
  ...
  // 重新获取所有地区选项（因为已经被替换）
  const newLocationOptions = document.querySelectorAll('.location-option');
  newLocationOptions.forEach(option => {
    // 为每个新的地区选项添加点击事件
    option.addEventListener('click', () => {
      selectLocation(option);
    });
  ...
  if (searchButton) {
    ...
    // 重新绑定搜索按钮事件（如果需要）
    document.getElementById('search-button').addEventListener('click', searchCommission);
  }
}

// 辅助函数：预加载常用图片，提高切换后的加载速度
function preloadImages() {
  ...
  // 预加载其他常用图标
  const commonIcons = ['like.png', 'dislike.png'];
...
function addClassicalChineseDecorativeElements() {
  // 先清理可能已存在的装饰元素
  document.querySelectorAll('.classical-svg, .classical-tip').forEach(el => el.remove());
  
  // 1. 为搜索区域添加装饰元素
  const searchContainer = document.querySelector('.search-container');
  if (searchContainer) {
    // 添加左侧云纹SVG
    const leftCloud = document.createElement('svg');
    leftCloud.className = 'classical-svg left-cloud';
    leftCloud.setAttribute('viewBox', '0 0 100 50');
    leftCloud.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
    leftCloud.innerHTML = `
      <path d="M0,25 Q25,0 50,25 T100,25" fill="none" stroke="#8B4513" stroke-width="2" opacity="0.7"/>
      <path d="M0,35 Q25,10 50,35 T100,35" fill="none" stroke="#8B4513" stroke-width="1.5" opacity="0.5"/>
    `;
    searchContainer.appendChild(leftCloud);
    
    // 添加右侧竹子SVG
    const rightBamboo = document.createElement('svg');
    rightBamboo.className = 'classical-svg right-bamboo';
    rightBamboo.setAttribute('viewBox', '0 0 50 100');
    rightBamboo.setAttribute('xmlns', 'http://www.w3.org/2000/svg');
    rightBamboo.innerHTML = `
      <line x1="25" y1="0" x2="25" y2="100" stroke="#8B4513" stroke-width="2" opacity="0.7"/>
      <path d="M20,10 L30,10 M20,30 L30,30 M20,50 L30,50 M20,70 L30,70 M20,90 L30,90" 
        stroke="#8B4513" stroke-width="1.5" opacity="0.7"/>
    `;
    searchContainer.appendChild(rightBamboo);
    
    // 决定是使用预设格言还是生成新格言
    let randomSaying;
    // 25%概率生成新格言
    if (Math.random() < 0.25) {
      randomSaying = generateClassicalSaying();
    } else {
      // 从格言库中随机选择
      randomSaying = classicalSayings[Math.floor(Math.random() * classicalSayings.length)];
    }
    
    // 添加古风提示语
    const classicalTip = document.createElement('div');
    classicalTip.className = 'classical-tip';
    classicalTip.textContent = randomSaying;
    searchContainer.appendChild(classicalTip);
  }
  
  // 2. 为每日限额区域添加装饰元素
  const limitsSection = document.querySelector('.daily-limits-section');
  if (limitsSection) {
    // 添加标题装饰
    const limitsTitleDecor = document.createElement('div');
    limitsTitleDecor.className = 'limits-title-decor';
    limitsTitleDecor.innerHTML = `
      <svg class="classical-svg limits-decor" viewBox="0 0 100 10" xmlns="http://www.w3.org/2000/svg">
        <path d="M0,5 H30 M70,5 H100" stroke="#8B4513" stroke-width="1" opacity="0.6"/>
        <circle cx="50" cy="5" r="3" fill="#8B4513" opacity="0.6"/>
        <circle cx="35" cy="5" r="1" fill="#8B4513" opacity="0.6"/>
        <circle cx="65" cy="5" r="1" fill="#8B4513" opacity="0.6"/>
      </svg>
    `;
    
    // 将装饰元素插入到标题之前
    const limitsTitle = limitsSection.querySelector('h3');
    if (limitsTitle) {
      limitsSection.insertBefore(limitsTitleDecor, limitsTitle);
    }
    
    // 从格言库中随机选择一条提示语（确保与搜索框不同）
    let randomLimitsSaying;
    const searchTip = document.querySelector('.search-container .classical-tip');
    const searchTipText = searchTip ? searchTip.textContent : '';
    
    // 33%概率生成新格言，否则从库中选择
    if (Math.random() < 0.33) {
      do {
        randomLimitsSaying = generateClassicalSaying();
      } while (randomLimitsSaying === searchTipText); // 避免与搜索框重复
    } else {
      // 避免与搜索框中的格言重复
      do {
        randomLimitsSaying = classicalSayings[Math.floor(Math.random() * classicalSayings.length)];
      } while (randomLimitsSaying === searchTipText && classicalSayings.length > 1);
    }
    
    // 添加古风提示语
    const limitsTip = document.createElement('div');
    limitsTip.className = 'classical-tip limits-tip';
    limitsTip.textContent = randomLimitsSaying || '量入为出，有节有度';
    limitsSection.appendChild(limitsTip);
  }
}
...
function translateToClassicalChinese(text) {
  ...
  const specialPhrases = {
    ...
    '下次刷新时间：': '下回更新之时：',
    '您今天已经发布了': '阁下今日已颁布',
    '个委托，请明天再来发布': '册令书，请明日再来颁布',
    '您今天已经发送了': '阁下今日已传送',
    '条留言，请明天再来留言': '条留字，请明日再来留字',
    '留言次数已达上限': '留字次数已达极限',
    '委托已成功发出': '令书已成功传出',
    '委托已成功删除': '令书已成功除去',
    '找到 ': '觅得 ',
...
function handleAdditionalFiles(event) {
  ...
  // 设置处理状态为true
  isProcessingAdditionalFiles = true;
  
  // 禁用发布按钮
  document.getElementById('publish-button').disabled = true;
  
  // 显示处理提示
  const loadingToast = showToast('正在处理文件...');
  ...
  files.forEach(file => {
    ...
    reader.onerror = function(error) {
      ...
      showCustomAlert('文件处理失败，请重试', '处理错误');
  ...
  function checkAllProcessed() {
    if (processedCount >= files.length) {
      // 设置处理状态为false
      isProcessingAdditionalFiles = false;
