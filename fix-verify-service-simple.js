...
// 保存修复脚本
fs.writeFileSync('fix-verify-simple.sh', fixScript);
console.log('已创建简化验证服务修复脚本');

// 上传并执行修复脚本
try {
  console.log('上传并执行修复脚本...');
  execSync('scp -i "C:\\Users\\34260\\.ssh\\id_rsa" fix-verify-simple.sh root@8.155.16.247:/tmp/');
  execSync('ssh -i "C:\\Users\\34260\\.ssh\\id_rsa" root@8.155.16.247 "chmod +x /tmp/fix-verify-simple.sh && /tmp/fix-verify-simple.sh"');
  console.log('验证服务已修复');
} catch (err) {
