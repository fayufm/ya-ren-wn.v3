...
// 修改admin-api.js中的验证逻辑
if (adminApiContent.includes('router.post(\'/verify\'')) {
  // 替换验证逻辑，使用明文密码而不是哈希
  let updatedApiContent = adminApiContent.replace(
    /if \(username !== adminConfig\.username\) {[\s\S]*?if \(inputHash !== adminConfig\.passwordHash\) {[\s\S]*?}/g,
  ...
  fs.writeFileSync(adminApiPath, updatedApiContent, 'utf8');
